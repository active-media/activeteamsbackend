# utils_annotated.py - annotated utility helpers for auth and general functions
import os
import secrets
...
# CONFIG
# ==============================
# JWT secret and algorithm settings with defaults
...
# HTTP bearer schema for FastAPI dependencies
...
def hash_password(password: str) -> str:
    # Hash a plain password with bcrypt and return the hash
...
def create_access_token(data: Dict[str, Any], expires_delta: Optional[timedelta] = None) -> str:
    # Create a JWT token including iat and exp claims
    to_encode = data.copy()
    now = datetime.utcnow()
...
def create_refresh_token() -> Dict[str, str]:
    # Generate a new refresh token (id + plain secret + bcrypt hash + expiry)
...
def decode_access_token(token: str) -> Dict[str, Any]:
    # Decode and validate a JWT token, return payload or raise HTTPException
    try:
...
async def refresh_access_token(refresh_token_id: str, refresh_token: str) -> Dict[str, Any]:
    # Validate stored refresh token and rotate to a new one, returning tokens
...
def create_password_reset_token(email: str, expires_delta: timedelta = timedelta(minutes=30)) -> str:
    # Create a JWT token for password reset with subject set to email
...
def require_role(*allowed_roles: str):
    # Returns a dependency that checks for allowed roles in the token
