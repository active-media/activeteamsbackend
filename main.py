...
#     """
#     Get cached people data for fast signup form
#     """
#     try:
...
#             current_time < people_cache["expires_at"] and 
#             not people_cache["is_loading"]):
            
#             print(f"✅ Returning cached people data ({len(people_cache['data'])} people)")
#             return {
...
#             try:
#                 print("🔄 Cache miss - refreshing people data...")
                
#                 # Fetch minimal people data for signup form
#                 projection = {
#                     "_id": 1,
#                     "Name": 1,
#                     "Surname": 1,
...
#                 cursor = people_collection.find({}, projection).limit(2000)  # Increased limit
#                 people_list = []
...
#                 # Update cache
#                 people_cache["data"] = people_list
#                 people_cache["last_updated"] = current_time.isoformat()
#                 people_cache["expires_at"] = (current_time + timedelta(minutes=CACHE_DURATION_MINUTES)).isoformat()
#                 people_cache["is_loading"] = False
                
#                 print(f"✅ Cache refreshed with {len(people_list)} people")
                
#                 return {
#                     "cached_data": people_list,
#                     "cached_at": people_cache["last_updated"],
#                     "expires_at": people_cache["expires_at"],
#                     "source": "database"
#                 }
...
#         else:
#             # Cache is currently being refreshed, wait a bit
...
#         people_cache["last_updated"] = None
#         people_cache["expires_at"] = None
#         people_cache["is_loading"] = False
        
#         print("🔄 Manual cache refresh triggered")
        
#         # Call the cache endpoint to refresh
#         return await get_cached_people()
...
# # Update the cache to handle unlimited people with pagination
# CACHE_DURATION_MINUTES = 30  # Cache for 30 minutes
# MAX_CACHE_SIZE = 50000  # Increased cache size limit

# @app.get("/people/ultra-fast")
# async def get_people_ultra_fast():
#     """
#     Ultra-fast people endpoint with minimal data for signup form
#     Optimized for unlimited records with pagination
#     """
#     try:
#         start_time = datetime.utcnow()
#         print("🚀 ULTRA-FAST endpoint called - fetching ALL people...")
        
#         # Only fetch essential fields for signup - minimal data transfer
#         projection = {
#             "_id": 1,
#             "Name": 1,
#             "Surname": 1,
...
#                 "key": {"$toString": "$_id"}
#             }}
#             # REMOVED the limit to get ALL people
#         ]
        
#         cursor = people_collection.aggregate(pipeline)
#         people_list = await cursor.to_list(length=None)  # No limit - get all
        
#         end_time = datetime.utcnow()
#         duration = (end_time - start_time).total_seconds()
        
#         print(f"⚡ ULTRA-FAST: Returned {len(people_list)} people in {duration:.2f} seconds")
...
# @app.get("/people/batch")
# async def get_people_batch(
#     page: int = Query(1, ge=1),
#     per_page: int = Query(2000, ge=500, le=5000)  # Increased batch size
# ):
#     """
#     Batch endpoint for loading people in chunks
#     Frontend can call this repeatedly until all data is loaded
#     """
#     try:
...
#         has_more = (skip + len(people_list)) < total_count
#         loaded_so_far = skip + len(people_list)
        
#         print(f"📦 Batch {page}: {len(people_list)} people (Total: {loaded_so_far}/{total_count})")
...
#                 "has_more": has_more,
#                 "progress_percentage": round((loaded_so_far / total_count) * 100, 1) if total_count > 0 else 0
#             }
#         }
        
#     except Exception as e:
#         print(f"❌ Error in batch endpoint: {str(e)}")
#         return {
#             "success": False,
#             "error": str(e),
#             "results": []
...
# # async def get_cached_people():
# #     """
# #     Get cached people data for fast signup form - UNLIMITED VERSION
# #     """
# #     try:
# #         # Check if cache is still valid
# #         current_time = datetime.utcnow()
...
# #             people_cache["expires_at"] and 
# #             current_time < people_cache["expires_at"] and 
# #             not people_cache["is_loading"] and
# #             len(people_cache["data"]) > 0):
...
# #             people_cache["is_loading"] = True
# #             try:
# #                 print("🔄 Cache miss - refreshing ALL people data...")
# #                 start_time = datetime.utcnow()
                
# #                 all_people_data = []
                
# #                 # Try ultra-fast endpoint first (no limits)
# #                 try:
# #                     print("⚡ Attempting ultra-fast load of ALL people...")
# #                     ultra_fast_response = await get_people_ultra_fast()
# #                     if ultra_fast_response["success"]:
# #                         all_people_data = ultra_fast_response["results"]
# #                         print(f"⚡ Ultra-fast loaded {len(all_people_data)} people")
# #                     else:
# #                         raise Exception("Ultra-fast endpoint failed")
                        
# #                 except Exception as ultra_error:
# #                     print("⚠️ Ultra-fast failed, using batch loading for ALL people...")
# #                     # Fallback to batch loading - load EVERYTHING
# #                     page = 1
# #                     has_more = True
# #                     total_loaded = 0
                    
# #                     while has_more:
# #                         batch_response = await get_people_batch(page=page, per_page=2000)
# #                         if batch_response["success"]:
# #                             batch_data = batch_response["results"]
# #                             all_people_data.extend(batch_data)
# #                             total_loaded += len(batch_data)
                            
# #                             pagination = batch_response["pagination"]
# #                             has_more = pagination["has_more"]
# #                             page += 1
                            
# #                             print(f"📦 Batch {page-1}: {len(batch_data)} people (Total: {total_loaded}, Progress: {pagination['progress_percentage']}%)")
                            
# #                             # Safety: break if we're loading too much (unlikely but safe)
# #                             if total_loaded > 100000:  # 100k safety limit
# #                                 print("⚠️ Reached safety limit of 100,000 people")
# #                                 break
# #                         else:
# #                             has_more = False
# #                             print("❌ Batch loading failed")
                
# #                 # Update cache with ALL people
...
# #                 print(f"✅ Cache refreshed with ALL {len(all_people_data)} people in {duration:.2f}s")
                
# #                 return {
# #                     "cached_data": all_people_data,
# #                     "cached_at": people_cache["last_updated"],
# #                     "expires_at": people_cache["expires_at"],
# #                     "source": "database",
...
# #         else:
# #             # Cache is currently being refreshed, return what we have
# #             if people_cache["data"]:
# #                 print("🕒 Cache is refreshing, returning existing data...")
# #                 return {
# #                     "cached_data": people_cache["data"],
# #                     "cached_at": people_cache["last_updated"],
# #                     "expires_at": people_cache["expires_at"],
...
#             "success": True,
#             "total_count": total_count,
#             "cache_count": len(people_cache["data"]),
#             "cache_updated": people_cache["last_updated"]
#         }
#     except Exception as e:
#         return {
#             "success": False,
...
#     """
#     cache_hit = (people_cache["last_updated"] and 
#                  people_cache["expires_at"] and 
#                  datetime.utcnow() < people_cache["expires_at"])
    
#     total_db_count = await people_collection.count_documents({})
    
#     return {
#         "cache": {
#             "size": len(people_cache["data"]),
#             "last_updated": people_cache["last_updated"],
#             "expires_at": people_cache["expires_at"],
#             "is_loading": people_cache["is_loading"],
#             "cache_hit": cache_hit,
#             "coverage_percentage": round((len(people_cache["data"]) / total_db_count) * 100, 1) if total_db_count > 0 else 0
#         },
#         "database": {
#             "total_people": total_db_count,
#             "collection_size": "N/A"
#         },
#         "system": {
...
#     # ✅ USE CACHED DATA FOR LEADER ASSIGNMENT
#     inviter_full_name = user.invited_by.strip()
#     leader1 = ""
#     leader12 = ""
#     leader144 = ""
...
#     if inviter_full_name:
#         print(f"🔍 Looking for inviter in cache: '{inviter_full_name}'")
        
#         # Search in cached data first (much faster)
#         cached_inviter = None
...
#         if cached_inviter:
#             print(f"✅ Found inviter in cache: {cached_inviter.get('FullName')}")
...
#             logger.info(f"Leader hierarchy set for {email}: L1={leader1}, L12={leader12}, L144={leader144}, L1728={leader1728}")
#         else:
#             print(f"⚠️ Inviter '{inviter_full_name}' not found in cache, trying database...")
...
#             if inviter:
#                 # Get the inviter's leader hierarchy from database
#                 inviter_leader1 = inviter.get("Leader @1", "")
#                 inviter_leader12 = inviter.get("Leader @12", "")
#                 inviter_leader144 = inviter.get("Leader @144", "")
#                 inviter_leader1728 = inviter.get("Leader @1728", "")
...
#                 logger.info(f"Leader hierarchy set for {email} (from DB): L1={leader1}, L12={leader12}, L144={leader144}, L1728={leader1728}")
#             else:
#                 logger.warning(f"Inviter '{inviter_full_name}' not found in people collection for {email}")
#                 # If inviter not found, still set them as Leader @1 as fallback
#                 leader1 = inviter_full_name
...
#         print(f"✅ Added new person to cache: {new_person_cache_entry['FullName']}")
...
async def is_user_leader_at_12(user_email: str, user_name: str) -> bool:
    ...
    try:
        ...
        # Check if ANY cells exist where they are Leader at 12
        leader_cell = await events_collection.find_one(leader_at_12_query)
        
        if leader_cell:
            print(f"✅ YES - {user_name} IS a Leader at 12 (found in cell: {leader_cell.get('Event Name')})")
            return True
        else:
            print(f"❌ NO - {user_name} is NOT a Leader at 12 (no cells found)")
            return False
            
    except Exception as e:
        print(f"Error checking Leader at 12 status: {e}")
        return False
...
async def get_cell_events(
    ...
    """
    FIXED: Personal/VIEW ALL toggle now works correctly for all user types
    """
    try:
        print(f"GET /events/cells - User: {current_user.get('email')}, Status: {status}")
        
        user_role = current_user.get("role", "user").lower()
        user_email = current_user.get("email", "")
        
        # Get user's name from their own cell event
        ...
        print(f"User name from events: {user_name}")
        
        # Check if user is ACTUALLY a Leader at 12 (appears as Leader at 12 in ANY event)
        is_actual_leader_at_12 = False
        if user_name:
            leader_at_12_check = await events_collection.find_one({
                "Event Type": "Cells",
                "$or": [
            ...
            is_actual_leader_at_12 = bool(leader_at_12_check)
        ...
        # FIXED: Admin users always use Admin logic, even if they are Leader at 12
        if user_role == "admin":
            if personal or show_personal_cells:
                print(f"ADMIN PERSONAL VIEW - Showing only personal cells")
                query["$or"] = [
                    {"Email": {"$regex": f"^{user_email}$", "$options": "i"}},
                ...
                print(f"ADMIN GLOBAL VIEW - No filters (showing all cells)")
                # Admin sees all cells (no additional filters)
                pass
        
        # FIXED: Leader at 12 VIEW (but NOT for Admin users)
        elif is_actual_leader_at_12:
            print(f"LEADER AT 12 VIEW for: {user_name}")
            
            if show_personal_cells or personal:
                # PERSONAL TAB: Only their own cells
                print("Showing ONLY personal cells")
                query["$or"] = [
                    {"Email": {"$regex": f"^{user_email}$", "$options": "i"}},
                    {"eventLeaderEmail": {"$regex": f"^{user_email}$", "$options": "i"}},
                    {"Leader": {"$regex": f"^{user_name}$", "$options": "i"}},
                ...
                # VIEW ALL TAB: Own cells + cells where they are Leader at 12
                ...
                query["$or"] = [
                    ...
                    # 2. Cells where they appear as "Leader at 12"
                    {"Leader at 12": {"$regex": f"^{user_name}$", "$options": "i"}},
                    {"Leader @12": {"$regex": f"^{user_name}$", "$options": "i"}},
                    {"leader12": {"$regex": f"^{user_name}$", "$options": "i"}},
                ]
        
        # Regular user and registrant view
        elif user_role == "registrant" or user_role == "user":
            print(f"{user_role.upper()} VIEW - Personal cells only")
            query["$or"] = [
                {"Email": {"$regex": f"^{user_email}$", "$options": "i"}},
                {"eventLeaderEmail": {"$regex": f"^{user_email}$", "$options": "i"}},
                {"Leader": {"$regex": f"^{user_name}$", "$options": "i"}},
            ...
            # Default: show only personal
            print(f"DEFAULT VIEW - Personal only")
            query["$or"] = [
                {"Email": {"$regex": f"^{user_email}$", "$options": "i"}},
                {"eventLeaderEmail": {"$regex": f"^{user_email}$", "$options": "i"}},
                {"Leader": {"$regex": f"^{user_name}$", "$options": "i"}},
        ...
        # Debug authorization results
        print(f"AUTHORIZATION DEBUG:")
        print(f"   - User: {user_email}")
        print(f"   - Role: {user_role}")
        print(f"   - Is Leader at 12: {is_actual_leader_at_12}")
        print(f"   - Personal filter: {personal}")
        print(f"   - Show personal cells: {show_personal_cells}")
        print(f"   - Query conditions: {len(query.get('$or', []))}")
        ...
        print(f"Final query: {json.dumps(query, indent=2, default=str)}")
        ...
        if events:
            print(f"Sample cells found:")
            for idx, evt in enumerate(events[:5]):
                print(f"   {idx+1}. {evt.get('Event Name')} - Leader: {evt.get('Leader')} - Leader@12: {evt.get('Leader @12')}")
        ...
        for event in events:
            try:
                ...
                # Generate instances from start_date to TODAY
                ...
                while instance_date <= today:
                    ...
                    # Check both the week_attendance status AND the event's did_not_meet field
                    did_not_meet = week_attendance.get("status") == "did_not_meet" or event.get("did_not_meet", False)
                    weekly_attendees = week_attendance.get("attendees", [])
                    has_weekly_attendees = len(weekly_attendees) > 0
                    
                    # Proper status calculation that matches what's stored
                    if did_not_meet:
                    ...
                    print(f"STATUS CHECK: {event_name} | Requested: {status} | Calculated: {event_status} | Week Status: {week_attendance.get('status')} | Event did_not_meet: {event.get('did_not_meet')}")
                    
                    # Apply status filter
                    if status and status != 'all' and status != event_status:
                        print(f"   FILTERED OUT - {event_name} doesn't match {status}")
                        instance_date += timedelta(days=7)
                        continue
                    else:
                        print(f"   INCLUDED - {event_name} matches {status}")
                    
                    persistent_attendees = event.get("persistent_attendees", [])
                    
                    # Build instance with proper status fields
                    instance = {
        ...
        print(f"Final breakdown - Total: {total_count}, Did Not Meet: {len([e for e in cell_instances if e.get('status') == 'did_not_meet'])}, Complete: {len([e for e in cell_instances if e.get('status') == 'complete'])}, Incomplete: {len([e for e in cell_instances if e.get('status') == 'incomplete'])}")
...
@app.get("/debug/leader-at-12-cells/{user_email}")
async def debug_leader_at_12_cells(user_email: str):
    """Debug what cells a Leader at 12 should see"""
    try:
        # Find user's name
        user_cell = await events_collection.find_one({
            "Event Type": "Cells",
        ...
        if not user_cell:
            return {"error": "User cell not found"}
        
        user_name = user_cell.get("Leader", "").strip()
        
        # Find cells where they are Leader at 12
        cells_as_leader12 = await events_collection.find({
            "Event Type": "Cells",
            "$or": [
                {"Leader at 12": {"$regex": f"^{user_name}$", "$options": "i"}},
                {"Leader @12": {"$regex": f"^{user_name}$", "$options": "i"}},
        ...
        return {
            ...
            "is_leader_at_12": len(cells_as_leader12) > 0,
            "cells_where_leader_at_12": [
                {
                    "event_name": cell.get("Event Name"),
                    "leader": cell.get("Leader"),
                    "leader_at_12": cell.get("Leader @12") or cell.get("Leader at 12"),
                    "day": cell.get("Day")
                }
                for cell in cells_as_leader12
            ],
            "total_cells": len(cells_as_leader12)
        }
...
async def get_other_events(
    ...
    try:
        print(f"GET /events/other - Event Type: '{event_type}', Status: '{status}'")

        user_role = current_user.get("role", "user").lower()
        email = current_user.get("email", "").lower()

        print(f"User Role: {user_role}, Personal filter from request: {personal}")

        # NO DATE FILTERING - Show ALL events
        print("DATE FILTERING DISABLED - Showing ALL events")
        ...
        # ===== EVENT TYPE FILTER - FIXED FOR YOUR DATA =====
        if event_type and event_type.lower() != 'all':
            print(f"Filtering for eventTypeName: '{event_type}'")
            
            # CRITICAL FIX: Only use eventTypeName field since that's what your data uses
            event_type_query = {
                "eventTypeName": event_type  # Exact match on the field your data actually uses
            }
            
            query = {"$and": [query, event_type_query]}
            print(f"Using exact eventTypeName filter: '{event_type}'")

        # ===== PERSONAL FILTER - FIXED =====
        # ONLY apply personal filter if explicitly requested (personal=true)
        # Registrants should see ALL events for event types, not just personal ones
        if personal:  # Changed from: if personal or user_role in ["user", "registrant"]
            print(f"Personal filter explicitly requested for: {email}")
            personal_conditions = [
                {"userEmail": {"$regex": email, "$options": "i"}},
                {"eventLeaderEmail": {"$regex": email, "$options": "i"}},
                {"eventLeader": {"$regex": f"^{email}$", "$options": "i"}},
            ]
            
            query = {"$and": [query, {"$or": personal_conditions}]}
        else:
            print(f"No personal filter - showing ALL events for {user_role}")
        ...
        if search and search.strip():
            ...
            search_query = {
                "$or": [
                    {"Event Name": {"$regex": search_term, "$options": "i"}},
                    {"eventName": {"$regex": search_term, "$options": "i"}},
                    {"eventLeader": {"$regex": search_term, "$options": "i"}},
        ...
        print(f"Found {len(events)} events after query")

        # ===== DEBUG: SHOW WHAT WE FOUND =====
        if events:
            print("EVENTS FOUND:")
            for i, event in enumerate(events):
                event_name = event.get("eventName") or event.get("Event Name", "Unnamed")
                actual_event_type = event.get("eventTypeName") or event.get("eventType") or event.get("Event Type", "Unknown")
                event_date = event.get("date") or event.get("Date Of Event") or event.get("eventDate", "No date")
                event_status = event.get("status", "unknown")
                
                print(f"  {i+1}. {event_name}")
                print(f"     Type: {actual_event_type}")
                print(f"     Date: {event_date}")
                print(f"     Status: {event_status}")
                print(f"     Leader: {event.get('eventLeader')}")
        else:
            print("NO EVENTS FOUND WITH CURRENT QUERY")

        other_events = []

        for event in events:
            try:
                event_name = event.get("eventName") or event.get("Event Name", "")
                event_type_value = event.get("eventTypeName") or event.get("eventType") or event.get("Event Type", "Event")
                ...
                if isinstance(event_date_field, datetime):
                    ...
                    try:
                        ...
                        # If date parsing fails, use created_at date
                        event_date = event.get("created_at", datetime.now()).date() if isinstance(event.get("created_at"), datetime) else datetime.now().date()
                else:
                    # If no date, use created_at date
                    event_date = event.get("created_at", datetime.now()).date() if isinstance(event.get("created_at"), datetime) else datetime.now().date()

                # FIXED STATUS CALCULATION - Use the status field from your data
                # Your events already have status: "complete", "open", etc.
                event_status_from_db = event.get("status", "incomplete")
                
                # Map your status values to the expected ones
                status_mapping = {
                    "complete": "complete",
                    "open": "incomplete", 
                ...
                event_status = status_mapping.get(event_status_from_db, "incomplete")
                
                # Check if it's marked as did_not_meet
                did_not_meet = event.get("did_not_meet", False) or event_status == "did_not_meet"
                
                # If did_not_meet is True, override status
                if did_not_meet:
                    event_status = "did_not_meet"

                # Apply status filter
                if status and status != 'all' and status != event_status:
                    print(f"FILTERED OUT: {event_name} - Status {event_status} doesn't match filter {status}")
                    continue

                print(f"INCLUDING: {event_name} | Status: {event_status} | DB Status: {event_status_from_db}")
                ...
                instance = {
                    ...
                    "eventLeaderName": event.get("eventLeader") or event.get("eventLeaderName", ""),
                    "eventLeaderEmail": event.get("userEmail") or event.get("eventLeaderEmail", ""),
                    "leader1": event.get("leader1", ""),
                    "leader12": event.get("leader12", ""),
                    "day": event.get("day") or event.get("Day") or "One-time",
                    ...
                    "Status": event_status.replace("_", " ").title(),
                    "did_not_meet": did_not_meet,
                    "_is_overdue": False,  # Disable for now
        ...
        print(f"FINAL RESULTS for '{event_type}':")
        print(f"   Total events: {total_count}")
        print(f"   Complete: {len([e for e in other_events if e.get('status') == 'complete'])}")
        print(f"   Did Not Meet: {len([e for e in other_events if e.get('status') == 'did_not_meet'])}")
        print(f"   Incomplete: {len([e for e in other_events if e.get('status') == 'incomplete'])}")
        print(f"   Returning: {len(paginated_events)} events")
...
async def check_leader_at_12_status(current_user: dict = Depends(get_current_user)):
    """
    Check if current user is ACTUALLY a Leader at 12
    Frontend calls this to determine if they should see the toggle
    """
    try:
        user_email = current_user.get("email")
        user_name = f"{current_user.get('name', '')} {current_user.get('surname', '')}".strip()
        
        print(f"🔍 Checking Leader at 12 status for: {user_name} ({user_email})")
        
        # Check if they are Leader at 12 in ANY cell
        is_leader = await is_user_leader_at_12(user_email, user_name)
        
        # Count how many cells they oversee
        cells_count = 0
        if is_leader:
            leader_at_12_query = {
                "Event Type": "Cells",
            ...
            cells_count = await events_collection.count_documents(leader_at_12_query)
        
        print(f"✅ Result: is_leader_at_12={is_leader}, cells_count={cells_count}")
        
        return {
            "is_leader_at_12": is_leader,
            "user_name": user_name,
            "user_email": user_email,
            "cells_overseen": cells_count,
            "message": f"{user_name} oversees {cells_count} cells as Leader at 12" if is_leader else f"{user_name} is not a Leader at 12"
        }
        
    except Exception as e:
        print(f"❌ Error checking Leader at 12 status: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
