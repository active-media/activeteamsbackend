...
async def get_cell_events(
    ...
    show_inactive: Optional[bool] = Query(False),  # Add this parameter
    active_only: Optional[bool] = Query(True)  # Add this parameter for backward compatibility
):
    try:
        print("=" * 100)
        print("GET /events/cells REQUEST")
        print(f'THEIR NAME {firstName} {userSurname}')
        print(f'SHOW INACTIVE PARAMETER: {show_inactive}')
        print(f'ACTIVE ONLY PARAMETER: {active_only}')
        ...
        print(f"Role: {role}")
        print(f"Show inactive cells: {show_inactive}")
        ...
        # Determine if we should show inactive cells
        # Default: only show active cells unless explicitly requested
        should_show_inactive = show_inactive or not active_only
        
        # Add active status filtering
        if not should_show_inactive:
            # Only show active cells (active = true OR active field doesn't exist)
            query["$and"].append({
                "$or": [
                    {"active": True},
                    {"active": {"$exists": False}}  # Cells created before active feature are considered active
                ]
            })
            print("Filtering: Showing only active cells")
        else:
            # Show all cells including inactive ones
            print("Filtering: Showing all cells (including inactive)")
        ...
        for event in events:
            if event.get('active') is False:
                inactive_count += 1
                print(f"  INACTIVE CELL: {event.get('Event Name', 'Unknown')} - ID: {event.get('_id')}")
            else:
                active_count += 1
        
        print(f"Active cells: {active_count}, Inactive cells: {inactive_count}")
        ...
        if len(events) > 0:
            ...
            for i, event in enumerate(events[:5]):
                ...
                active_status = "Active" if event.get('active') is not False else "INACTIVE"
                print(f"   {i+1}. {event.get('Event Name', 'N/A')} [{active_status}]")
                print(f"      Leader: {cell_leader}")
                print(f"      Leader@12: {leader_at_12}")
                print(f"      Status from DB: {event.get('Status')}")
                print(f"      Active field: {event.get('active')}")
        ...
        for event in events:
            try:
                ...
                is_cell_inactive = event.get('active') is False
                if is_cell_inactive:
                    inactive_instances_count += 1
                    print(f"\nProcessing INACTIVE cell: {event.get('Event Name', 'Unknown')}")
                else:
                    print(f"\nProcessing ACTIVE cell: {event.get('Event Name', 'Unknown')}")
                ...
                print(f"  Active Status: {'INACTIVE' if is_cell_inactive else 'Active'}")
                ...
                for week_offset in range(4):
                    ...
                    instance = {
                        ...
                        "active": not is_cell_inactive,  
                        "_is_cell_inactive": is_cell_inactive  
                    }
        ...
        print(f"  Inactive cells processed: {inactive_instances_count}")
        ...
        for inst in cell_instances:
            ...
            if inst.get('_is_cell_inactive'):
                inactive_instance_count += 1
            else:
                active_instance_count += 1
        
        print(f"  Status breakdown: {status_counts}")
        print(f"  Active instances: {active_instance_count}")
        print(f"  Inactive instances: {inactive_instance_count}")
        
        print("\nSample instances in response:")
        for i, inst in enumerate(paginated[:5]):
            active_status = "INACTIVE" if inst.get('_is_cell_inactive') else "Active"
            print(f"  {i+1}. {inst['eventName']} [{active_status}]")
        ...
        return {
            ...
            "cell_stats": {
                ...
                "inactive": inactive_instance_count,
                "total": total_count,
                "unique_cells_active": active_count,
                "unique_cells_inactive": inactive_count
            },
            "user_info": {
                "name": user_name,
                "email": user_email,
...
@app.put("/cells/{cell_id}/active")
async def toggle_cell_active_status(
    cell_id: str = Path(..., description="The ID of the cell to update"),
    active: bool = Body(..., embed=True, description="Set to true to activate, false to deactivate"),
    current_user: dict = Depends(get_current_user)
):
    """
    Toggle the active status of a cell.
    When deactivated, the cell won't show in listings until activated again.
    Only admins and Leader 1 can toggle this status.
    """
    try:
        print(f"=== TOGGLE CELL ACTIVE STATUS ===")
        print(f"Requested cell_id: {cell_id}")
        print(f"Active status to set: {active}")
        print(f"User: {current_user.get('email')}, Role: {current_user.get('role')}")
        
        # Check permissions
        role = current_user.get("role", "user").lower()
        if role not in ["admin", "leader1"]:
            print(f"Permission denied: User role '{role}' not authorized")
            raise HTTPException(
                status_code=403, 
                detail="Only administrators and Leader 1 can change cell active status"
            )
        
        # First, let's check if this is a compound ID (with underscore)
        original_search_id = cell_id
        base_cell_id = cell_id
        is_compound_id = "_" in cell_id
        
        if is_compound_id:
            parts = cell_id.split("_")
            base_cell_id = parts[0]  # Use the first part (the actual ObjectId)
            print(f"Compound ID detected. Using base ID: {base_cell_id}")
        ...
        search_strategies = [
            ("ObjectId (base)", lambda: events_collection.find_one({"_id": ObjectId(base_cell_id)})),
            ("ObjectId (full)", lambda: events_collection.find_one({"_id": ObjectId(original_search_id)})),
            ("String _id", lambda: events_collection.find_one({"_id": original_search_id})),
            ("ID field", lambda: events_collection.find_one({"ID": original_search_id})),
            ("UUID", lambda: events_collection.find_one({"UUID": original_search_id})),
        ]
        
        for method_name, search_func in search_strategies:
            try:
                found_cell = await search_func()
                if found_cell:
                    cell = found_cell
                    search_method = method_name
                    print(f"Found cell using {method_name}: {cell.get('Event Name', 'N/A')}")
                    break
            except Exception as e:
                print(f"Search method {method_name} failed: {e}")
                continue
        
        # If still not found, try searching by name
        if not cell:
            print(f"Trying to find cell by name containing: {original_search_id}")
            
            # Extract potential name from the ID (sometimes IDs are used in names)
            search_cells = await events_collection.find({
                "$or": [
                    {"Event Name": {"$regex": original_search_id, "$options": "i"}},
                    {"eventName": {"$regex": original_search_id, "$options": "i"}},
                ]
            }).to_list(length=5)
            ...
            if search_cells:
                ...
                print(f"Found cell by name search: {cell.get('Event Name', 'N/A')}")
        
        if not cell:
            # Let's check what's actually in the database
            print(f"\n=== CHECKING DATABASE FOR CELLS ===")
            
            # Get all documents that might be cells
            potential_cells = await events_collection.find({
                "$or": [
                    {"Event Type": {"$regex": "cell", "$options": "i"}},
                    {"eventType": {"$regex": "cell", "$options": "i"}},
                    {"eventTypeName": {"$regex": "cell", "$options": "i"}},
                    {"Event Name": {"$regex": "cell", "$options": "i"}},
                ]
            }).to_list(length=20)
            
            print(f"Found {len(potential_cells)} potential cells in database")
            
            # Also check if any document has this ID in any field
            all_docs_with_id = await events_collection.find({
                "$or": [
                    {"_id": original_search_id},
                    {"ID": original_search_id},
                    {"UUID": original_search_id},
                ]
            }).to_list(length=10)
            
            print(f"Found {len(all_docs_with_id)} documents with ID {original_search_id} in any field")
            
            for i, doc in enumerate(all_docs_with_id):
                print(f"\nDocument {i+1}:")
                print(f"  _id: {doc.get('_id')}")
                print(f"  Type: {doc.get('Event Type', doc.get('eventType', 'Unknown'))}")
                print(f"  Name: {doc.get('Event Name', doc.get('eventName', 'N/A'))}")
            
            # Create a helpful error message
            error_msg = f"""
            Cell not found with ID: {original_search_id}
            
            Searched for:
            - ObjectId: {base_cell_id}
            - Original ID: {original_search_id}
            - In ID field
            - In UUID field
            
            Found {len(potential_cells)} potential cells in database.
            First 5 cells found:
            """
            
            for i, c in enumerate(potential_cells[:5]):
                error_msg += f"\n{i+1}. ID: {c.get('_id')}, Name: {c.get('Event Name', 'N/A')}"
            
            raise HTTPException(status_code=404, detail=error_msg)
        
        # Verify this is actually a cell
        event_type = (
            cell.get('Event Type') or 
            cell.get('eventType') or 
            cell.get('eventTypeName') or 
            ""
        ).lower()
        
        event_name = cell.get('Event Name') or cell.get('eventName') or "Unknown"
        
        if 'cell' not in event_type and 'cell' not in event_name.lower():
            print(f"WARNING: Document may not be a cell. Type: {event_type}, Name: {event_name}")
            # Ask for confirmation? Or just proceed?
        
        print(f"\nCell found successfully:")
        print(f"  Name: {event_name}")
        print(f"  Type: {event_type}")
        print(f"  Database ID: {cell['_id']}")
        print(f"  Current active: {cell.get('active', 'Not set (defaults to True)')}")
        ...
        if update_result.modified_count == 1:
            success_msg = f"Cell '{event_name}' {'activated' if active else 'deactivated'} successfully"
            print(success_msg)
            ...
            return {
                ...
                "cell_name": event_name,
                "active": active,
                "found_by": search_method,
                "previous_active": cell.get('active')
            }
        else:
            # Check if the value was already set
            current_active = cell.get('active')
            if current_active == active:
                return {
                    "success": True,
                    "message": f"Cell '{event_name}' already {'activated' if active else 'deactivated'}",
                    "cell_id": str(cell["_id"]),
                    "cell_name": event_name,
                    "active": active,
                    "already_set": True
                ...
                error_msg = f"Failed to update cell status. Modified count: {update_result.modified_count}"
                print(error_msg)
        ...
        print(f"Error toggling cell active status: {str(e)}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Server error: {str(e)}")

@app.put("/cells/bulk-active")
async def bulk_toggle_cells_active_status(
    cell_ids: List[str] = Body(..., description="List of cell IDs to update"),
    active: bool = Body(..., description="Active status to set"),
    current_user: dict = Depends(get_current_user)
):
    """
    Bulk toggle active status for multiple cells.
    Only admins and Leader 1 can perform this operation.
    """
    try:
        # Check permissions
        role = current_user.get("role", "user").lower()
        if role not in ["admin", "leader1"]:
            raise HTTPException(status_code=403, detail="Only administrators and Leader 1 can change cell active status")
        ...
        if invalid_ids:
            raise HTTPException(status_code=400, detail=f"Invalid cell IDs: {invalid_ids}")
        ...
        return JSONResponse(
            status_code=200,
            content={
                "message": f"{result.modified_count} cells {'activated' if active else 'deactivated'}",
        ...
        print(f"Error in bulk toggle: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/diagnose/cell/{cell_id}")
async def diagnose_cell(
    cell_id: str,
    current_user: dict = Depends(get_current_user)
):
    """Diagnostic endpoint to understand cell structure"""
    try:
        print(f"\n=== DIAGNOSE CELL: {cell_id} ===")
        
        # Get the cell from events collection
        cell = None
        try:
        ...
        if not cell:
            # Try as string
            cell = await events_collection.find_one({"_id": cell_id})
        
        if not cell:
            return {
                "found": False,
                "message": f"Cell with ID {cell_id} not found in events collection",
                "collection": "events",
                "search_id": cell_id
            }
        ...
        for key, value in cell.items():
            if key == "_id":
                cell_structure[key] = str(value)
            elif isinstance(value, (str, int, float, bool, type(None))):
                cell_structure[key] = value
            elif isinstance(value, list):
                cell_structure[key] = f"List with {len(value)} items"
            elif isinstance(value, dict):
                cell_structure[key] = f"Dict with {len(value)} keys"
            else:
                cell_structure[key] = str(type(value))
        
        # Check if it has the active field
        has_active = "active" in cell
        current_active = cell.get("active")
        ...
        return {
            ...
            "collection": "events",
            "cell_id": str(cell["_id"]),
            "cell_name": cell.get("Event Name") or cell.get("eventName") or "Unknown",
            "event_type": cell.get("Event Type") or cell.get("eventType") or cell.get("eventTypeName"),
            "has_active_field": has_active,
...
@app.get("/debug/cells/{search_id}")
async def debug_cell_search(
    search_id: str,
    current_user: dict = Depends(get_current_user)
):
    """Debug endpoint to search for cells by various IDs"""
    try:
        print(f"=== DEBUG SEARCH FOR CELL: {search_id} ===")
        
        # Try to find by _id
        try:
            cell_by_objectid = await events_collection.find_one({"_id": ObjectId(search_id)})
            if cell_by_objectid:
                print(f"Found by ObjectId: {cell_by_objectid.get('Event Name', 'N/A')}")
        except:
            print("Not a valid ObjectId")
        
        # Try by ID field
        cell_by_id_field = await events_collection.find_one({"ID": search_id})
        if cell_by_id_field:
            print(f"Found by ID field: {cell_by_id_field.get('Event Name', 'N/A')}")
        
        # Try by UUID
        cell_by_uuid = await events_collection.find_one({"UUID": search_id})
        if cell_by_uuid:
            print(f"Found by UUID: {cell_by_uuid.get('Event Name', 'N/A')}")
        
        # Search in event name
        cells_by_name = await events_collection.find({
            "Event Name": {"$regex": search_id, "$options": "i"}
        }).to_list(length=5)
        
        if cells_by_name:
            print(f"Found {len(cells_by_name)} cells by name search")
            for i, cell in enumerate(cells_by_name):
                print(f"  {i+1}. {cell.get('Event Name', 'N/A')} (ID: {cell.get('_id')})")
        
        # Get all cells to see what IDs exist
        all_cells = await events_collection.find({
            "$or": [
                {"Event Type": {"$regex": "^Cells$", "$options": "i"}},
                {"eventType": {"$regex": "^Cells$", "$options": "i"}},
        ...
        print(f"\n=== FIRST 20 CELLS IN DATABASE ===")
        for i, cell in enumerate(all_cells):
            print(f"{i+1}. _id: {cell.get('_id')}")
            print(f"   Name: {cell.get('Event Name', cell.get('eventName', 'N/A'))}")
            print(f"   ID field: {cell.get('ID', 'N/A')}")
            print(f"   UUID: {cell.get('UUID', 'N/A')}")
            print(f"   Event Type: {cell.get('Event Type', cell.get('eventType', 'N/A'))}")
            print(f"   Event Type Name: {cell.get('eventTypeName', 'N/A')}")
            print()
        
        return {
            "search_id": search_id,
            "found_by_objectid": bool(cell_by_objectid),
            "found_by_id_field": bool(cell_by_id_field),
            "found_by_uuid": bool(cell_by_uuid),
            "cells_by_name": len(cells_by_name),
